		...
		foreach ( $csv as $key => $value ) {
           ...
           if ( isset( $csv[$key+2] ) ) {
	         ...
	         $diff_high_open_price_percentage = $diff_open_low_price_percentage = 0;
           	 ...
           	 /*if ( ($csv[$key][5] > $csv[$key+1][5]) && ( $csv[$key][4] < $csv[$key+1][4] ) ) {
           	 	 //Volume Decreases Price Increases(Sell)
	             $diff_price = number_format((( $csv[$key+2][1] - $csv[$key+2][3] )/$csv[$key+2][1])*100, 2, '.', '');
	             $diff_close_price_percentage = number_format((($csv[$key+1][4] - $csv[$key][4])/$csv[$key][4])*100, 2, '.', '');
	             $diff_volume_percentage = number_format((($csv[$key][5] - $csv[$key+1][5])/$csv[$key][5])*100, 2, '.', '');
	             if ( $diff_close_price_percentage > 1 && $diff_volume_percentage > 50 ) {
		             print $csv[$key+2][0] . " Volume Decreases Price Increases(Sell) = " . $diff_price . " | CP = " . $diff_close_price_percentage . " | Vol = " . $diff_volume_percentage .  "\r\n";
	             }
             }
	         if ( ($csv[$key][5] > $csv[$key+1][5]) && ( $csv[$key][4] > $csv[$key+1][4] ) ) {
		         //Volume Decreases Price Decreases(Buy)
		         $diff_price = number_format((( $csv[$key+2][4] - $csv[$key+2][1] )/$csv[$key+2][1])*100, 2, '.', '');
		         $diff_close_price_percentage = number_format((($csv[$key][4] - $csv[$key+1][4])/$csv[$key][4])*100, 2, '.', '');
		         $diff_volume_percentage = number_format((($csv[$key][5] - $csv[$key+1][5])/$csv[$key][5])*100, 2, '.', '');
		         if ( $diff_close_price_percentage > 1 && $diff_volume_percentage > 50 ) {
			         print $csv[ $key + 2 ][0] . " Volume Decreases Price Decreases(Buy) = " . $diff_price . " | CP = " . $diff_close_price_percentage . " | Vol = " . $diff_volume_percentage . "\r\n";
		         }
	         }*/

	         /**********************************===================================================================********************/
	         /*if ( ($csv[$key][5] < $csv[$key+1][5]) && ( $csv[$key][4] < $csv[$key+1][4] ) ) {
		         //Volume Increases Price Increases(Buy)
		         //$diff_price = number_format((( $csv[$key+2][2] - $csv[$key+2][1] )/$csv[$key+2][1])*100, 2, '.', '');
		         $diff_close_price_percentage = number_format((($csv[$key+1][4] - $csv[$key][4])/$csv[$key][4])*100, 2, '.', '');
		         $diff_volume_percentage = number_format((($csv[$key+1][5] - $csv[$key][5])/$csv[$key][5])*100, 2, '.', '');
		         //if ( ($diff_close_price_percentage > 1 && $diff_close_price_percentage < 2) && $diff_volume_percentage > 50 ) {
		         if ( $csv[ $key + 1 ][0] == '24-Jan-2019' && $diff_close_price_percentage < 2 && $diff_volume_percentage >= 50 ) {
			         //print $csv[ $key + 2 ][0] . " Volume Increases Price Increases(Buy) = " . $diff_price . " | CP = " . $diff_close_price_percentage . " | Vol = " . $diff_volume_percentage . "\r\n";
			         print $csv[ $key + 1 ][0] . " Volume Increases Price Increases(Buy) "  . " | CP = " . $diff_close_price_percentage . " | Vol = " . $diff_volume_percentage . "\r\n";
		         }
	         }

	         if ( ($csv[$key][5] < $csv[$key+1][5]) && ( $csv[$key][4] > $csv[$key+1][4] ) ) {
		           //Volume Increases Price Decreases(Sell)
		           //$diff_price = number_format((( $csv[$key+2][1] - $csv[$key+2][3] )/$csv[$key+2][1])*100, 2, '.', '');
		           $diff_close_price_percentage = number_format((($csv[$key][4] - $csv[$key+1][4])/$csv[$key][4])*100, 2, '.', '');
		           $diff_volume_percentage = number_format((($csv[$key+1][5] - $csv[$key][5])/$csv[$key][5])*100, 2, '.', '');
		           //if ( ($diff_close_price_percentage > 1 && $diff_close_price_percentage < 2) && $diff_volume_percentage > 50 ) {
		           if ( $csv[ $key + 1 ][0] == '24-Jan-2019' && $diff_close_price_percentage < 2 &&  $diff_volume_percentage >= 50 ) {
		            //print $csv[ $key + 2 ][0] . " Volume Increases Price Decreases(Sell) = " . $diff_price . " | CP = " . $diff_close_price_percentage . " | Vol = " . $diff_volume_percentage . "\r\n";
		            print $csv[ $key + 1 ][0] . " Volume Increases Price Decreases(Sell) " . " | CP = " . $diff_close_price_percentage . " | Vol = " . $diff_volume_percentage . "\r\n";
		           }
	         }*/
	         /**********************************===================================================================********************/
