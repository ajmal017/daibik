	...
	if(!empty($json)) {
        ...
        /*if($data->length > 0 ) {
		 ...
		 if( !empty( $data->data[0] ) ) {
		    ...
		    $date = date('d-M-Y');
		    $open_price = $response->data[0]->openPrice;
		    $high_price = $response->data[0]->highPrice;
		    $low_price = $response->data[0]->lowPrice;
			$close_price = $response->data[0]->lastPrice;
		    $volume = str_replace(',', '', $response->data[0]->numberOfContractsTraded);
...
function updateData($data, $conn) {
  $sql = "UPDATE live_feed_futures SET
...
function previousPatternMatch( $symbol, $conn ) {
   ...
   $sql = "SELECT * FROM analysis_futures WHERE symbol ='". $symbol . "' LIMIT 1";
...
function dryDayCount($symbol, $conn) {
	...
	$sql = "SELECT * FROM volume_analysis_future WHERE symbol ='". $symbol . "'";
	$query=mysqli_query($conn,$sql);
	while ( $result = mysqli_fetch_array($query,MYSQLI_ASSOC) ) {
