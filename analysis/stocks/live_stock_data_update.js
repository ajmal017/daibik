...
const intervalObjUpdateVolumeRepeat = setInterval(updatePriceVolume, 80000);
...
function updatePriceVolume() {
  ...
  async.each(files, function(file, callback) {
    var stock = new Mimicry();
    stock.get('http://localhost/daibik/analysis/stocks/live_stock_data_update.php?symbol='+file.symbol.replace(/&/gi, function myFunction(x){return '_';}), function(err, data) {
      console.log(data);
    });
  });
}
