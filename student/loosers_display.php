...
if ($conn->connect_error ) {
	...
	$sql = "SELECT date_added FROM fo_loosers group by date_added order by date_added DESC";
...
<body>
<form action="loosers_display.php" method="POST">
	<label>Date</label>
	...
	<table>
		...
		<?php
		$sql_details = "SELECT * FROM fo_loosers WHERE date_added='". $selected ."' ORDER BY low_percentage DESC LIMIT 0,20";
		...
		while ( $result = mysqli_fetch_array($query_details,MYSQLI_ASSOC) ) {
			...
			<tr>
				...
				<td><?php print $result['low_percentage']; ?></td>
			</tr>
			<?php
		}
		?>
